...
export default function CustomerLedgerPage() {
    ...
    const { customers, transactions, customerPayments, deletePayment } = useTransactions();
    ...
    const handleExport = () => {
        const dataToExport = periodTransactions.map(t => ({
            ...
            'Payment Received': `${formatCurrency(t.debit)}${t.paymentMethods ? ` (${t.paymentMethods})` : ''}`,
        ...
        const summary = {
            ...
            'Payment Received': formatCurrency(totalCredit),
    ...
    const confirmDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            await deletePayment(transactionToDelete.id, "Customer", customerId);
    ...
    return (
        <>
            <Header title="Payment Dues" backHref="/sales/customers" />
            <main className="flex flex-1 flex-col gap-3 p-2 md:gap-4 md:p-4 print:p-0">
                <div className="hidden print:block mb-8 border-b-2 border-black pb-4 text-left">
                    <h1 className="text-3xl font-black uppercase tracking-tighter">Account Statement</h1>
                            ...
                            <div className="flex flex-col gap-3 print:gap-1">
                                <div className="flex items-center gap-3 print:gap-0">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground print:text-black print:text-[8px]">Customer Name</span>
                                        <h2 className="text-xl font-black text-[#064e3b] print:text-black print:text-2xl">{customer.name}</h2>
                                    </div>
                                </div>
                                {customer.address && (
                                    <div className="flex items-center gap-3 ml-1 print:ml-0 print:gap-0">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <p className="text-sm font-semibold text-slate-700 print:text-black">{customer.address}</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        <Card className="relative overflow-hidden border border-slate-200 shadow-sm bg-slate-50/30 print:bg-white print:border print:border-black print:shadow-none">
                            ...
                            <CardContent className="pb-6 relative z-10">
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 text-center">
                                    ...
                                    <div className="bg-white rounded-xl p-2 shadow-sm border border-slate-200">
                                        <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-0.5">Total Received</p>
                                    ...
                                    <TableRow className="bg-emerald-50/50">
                                        ...
                                        <TableHead className="text-right print:text-center">
                                            <div className="flex items-center justify-end gap-1 print:justify-center">
                                                <ShoppingCart className="h-4 w-4 text-sky-600" /> Sales
                                            </div>
                                        </TableHead>
                                        <TableHead className="text-right print:text-center">
                                            <div className="flex items-center justify-end gap-1 print:justify-center">
                                                <TrendingUp className="h-4 w-4 text-green-600" /> Payment Received
                                            </div>
                                        </TableHead>
                                    ...
                                    {periodTransactions.length === 0 ? (
                                        ...
                                        periodTransactions.map(t => (
                                            <TableRow key={t.id}>
                                                    ...
                                                    {t.amount > 0 ? (
                                                        <div className="flex items-center justify-end gap-2 print:justify-center">
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => {
                                                                    setSelectedTransactions(t.transactions.filter((tx: any) => tx.type === 'Sale'));
                                                ...
                                                <TableCell className="text-right font-bold text-primary print:text-center">{formatCurrency(t.closing)}</TableCell>
                                            </TableRow>
                                        ))
                                    )}
                                </TableBody>
                            ...
                            <TableBody>
                                {(() => {
                                    ...
                                    const grouped = selectedTransactions.reduce((acc: any[], curr) => {
                                        if (curr.type === 'Sale') {
                                            const existing = acc.find(t => t.billNumber === curr.billNumber && t.type === 'Sale');
                                            if (existing) {
                                                ...
                                                <>
                                                    ...
                                                    <TableCell>
                                                        <span className={cn(
                                                            ...
                                                            {tx.type === 'Sale' ? tx.payment : tx.type}
                                                        </span>
                                                    </TableCell>
                                                </>
                                            )}
                    ...
                    <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                        <AlertDialogContent className="z-[100]">
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    Are you sure you want to delete this payment of <strong>{transactionToDelete ? formatCurrency(transactionToDelete.amount) : ''}</strong>? This action will update the customer's balance and cannot be undone.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
